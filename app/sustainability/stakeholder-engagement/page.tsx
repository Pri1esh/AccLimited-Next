import { ErrorFallback, ImageBannerComponent, Layout, CustomHeading, RteData, StakeEngagementbtns } from '@components';
import { ENDPOINT } from '@api-manager';
import { getApiData, getMetadata } from '@utils/server';

export async function generateMetadata(): Promise<unknown> {
  const apiData = await getApiData(ENDPOINT.SSR.sustainabilityEngagement);
  const { data } = apiData;
...
const StakeholderEngagement = async () => {
  const apiData = await getApiData(ENDPOINT.SSR.sustainabilityEngagement);

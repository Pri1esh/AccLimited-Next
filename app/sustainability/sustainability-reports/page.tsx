import { ErrorFallback, ImageBannerComponent, Layout, CustomHeading, ArchiveSection } from '@components';
import { ENDPOINT } from '@api-manager';
import { getApiData, getMetadata } from '@utils/server';

export async function generateMetadata(): Promise<unknown> {
  const apiData = await getApiData(ENDPOINT.SSR.sustainabilityReports);
  const { data } = apiData;

  return getMetadata(data?.main?.SEO?.fields);
}

const SustainabilityReports = async () => {
  const apiData = await getApiData(ENDPOINT.SSR.sustainabilityReports);
  ...
  return (
    ...
    >
      <>
        ...
        {main?.SusReports && <ArchiveSection aspect='widest' compData={main?.SusReports} />}
      </>
    </Layout>
  );
};

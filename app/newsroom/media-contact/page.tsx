...
import { MediaContactSection, CustomHeading, ErrorFallback, ImageBannerComponent, Layout } from '@components';
import { getApiData, getMetadata } from '@utils/server';

export async function generateMetadata(): Promise<unknown> {
  const apiData = await getApiData(ENDPOINT.SSR.mediaContact);
  const { data } = apiData;

  return getMetadata(data?.main?.SEO?.fields);
}

const MediaContact = async () => {
  const apiData = await getApiData(ENDPOINT.SSR.mediaContact);
  ...
  return (
    ...
    >
      <>
        ...
        {main?.MediaContactDetails && <MediaContactSection compData={main?.MediaContactDetails} />}

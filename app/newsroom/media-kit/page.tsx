...
import ShareHolderInfo from 'src/components/shareHoldersInformation';

export async function generateMetadata(): Promise<unknown> {
  const apiData = await getApiData(ENDPOINT.SSR.mediaKit);
  const { data } = apiData;
  ...
  return (
    ...
    >
      <>
        ...
        {main?.Pdfs && <ShareHolderInfo compData={main?.Pdfs?.fields} />}
      </>
    </Layout>
  );
};

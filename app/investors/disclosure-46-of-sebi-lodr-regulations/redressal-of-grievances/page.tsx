...
import { CustomHeading, ErrorFallback, ImageBannerComponent, Layout, BrandStorySection } from '@components';
import { getApiData, getMetadata } from '@utils/server';

export async function generateMetadata(): Promise<unknown> {
  const apiData = await getApiData(ENDPOINT.SSR.disclosure46_grievances);
  const { data } = apiData;

  return getMetadata(data?.main?.SEO?.fields);
}

const Grivance = async () => {
  const apiData = await getApiData(ENDPOINT.SSR.disclosure46_grievances);

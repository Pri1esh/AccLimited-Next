...
export async function generateMetadata(): Promise<unknown> {
  const apiData = await getApiData(ENDPOINT.SSR.disclosure46);
  const { data } = apiData;

  return getMetadata(data?.main?.SEO?.fields);
}

const Disclosure46 = async () => {
  const apiData = await getApiData(ENDPOINT.SSR.disclosure46);
  ...
  return (
    ...
    >
      ...
      <>
        ...
        {/* {main?.DisclosureUnder30A &&
          <DisclousreSection compData={main?.DisclosureUnder30A?.fields?.[0]} />
        } */}
         {main?.CommonPageData && <CustomHeading compData={main?.CommonPageData?.fields} />}
         {main?.DisclosureUnder30A && <ShareHolderInfo compData={main?.DisclosureUnder30A?.fields}/>}

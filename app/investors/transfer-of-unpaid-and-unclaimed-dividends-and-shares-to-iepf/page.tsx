...
export async function generateMetadata(): Promise<unknown> {
  const apiData = await getApiData(ENDPOINT.SSR.transferOfUnpaid);
  const { data } = apiData;

  return getMetadata(data?.main?.SEO?.fields);
}

const TranferOfUnpaind = async () => {
  const apiData = await getApiData(ENDPOINT.SSR.transferOfUnpaid);
  ...
  return (
    ...
    >
     ...
     {main?.IEPF && <ShareHolderInfo compData={main?.IEPF?.fields}/>}
...
export default TranferOfUnpaind 
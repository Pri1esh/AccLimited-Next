import { CustomHeading, ErrorFallback, ImageBannerComponent, Layout, AlternateCardSection } from '@components';
import { ENDPOINT } from '@api-manager';
import { getApiData, getMetadata } from '@utils/server';

export async function generateMetadata(): Promise<unknown> {
  const apiData = await getApiData(ENDPOINT.SSR.industryLeadership);
  const { data } = apiData;
...
const IndustryLeadership = async () => {
  const apiData = await getApiData(ENDPOINT.SSR.industryLeadership);
  ...
  return (
    ...
    >
      <>
        ...
        {main?.CommonPageData && <AlternateCardSection compData={main?.CardsData} />}
